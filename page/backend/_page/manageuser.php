<?php
	if(isset($_GET['id']) && $_GET['id'] != NULL && $_GET['id'] != "")
	{
		...
		if($query_user->num_rows != 0)
		{
			...
			{
				if(
				$_POST['email'] != NULL && $_POST['email'] != "" && $_POST['points'] != NULL && $_POST['points'] != "" && $_POST['eve'] != NULL && $_POST['eve'] != "" && $_POST['rp'] != NULL && $_POST['rp'] != "" && $_POST['status'] != NULL && $_POST['status'] != "")
				{
					$sql_edit = 'UPDATE authme SET email = "'.$_POST['email'].'", points = "'.$_POST['points'].'", rp = "'.$_POST['rp'].'", eve = "'.$_POST['eve'].'", status = "'.$_POST['status'].'" WHERE id = "'.$user_f['id'].'"';
			...
			?>
				...
				<form name="edit_user" method="POST">
					<div class="row">
						...
						<div class="col-md-3 mb-3">
				            <label for="points">Rp Points</label>
				            <input type="text" class="form-control" id="rp" name="rp" value="<?php echo $user_f['rp']; ?>">
				        </div>
						<div class="col-md-3 mb-3">
				            <label for="points">Event Points</label>
				            <input type="text" class="form-control" id="eve" name="eve" value="<?php echo $user_f['eve']; ?>">
				        </div>
				        <div class="col-md-3 mb-3">
			...
			<table class="table table-default table-striped table-condenseds">
				<thead>
					<tr>
						...
						<th style="background-color: #FFF;" class="text-left text-dark">ชื่อตัวละคร</th>
						<th style="background-color: #FFF;" class="text-left text-dark">ไอพี</th>
						<th style="background-color: #FFF;" class="text-left text-dark">tp/rp/ep</th>
						<th style="background-color: #FFF;" class="text-left text-dark">สถานะ</th>
					</tr>
				</thead>
				<tbody>
					<?php
						...
						if($query_user_w->num_rows != 0)
						{
							while($user_w = $query_user_w->fetch_assoc())
							{
								...
								echo '
									<tr>
										<td class="text-white" class="text-left">'.$user_w['id'].'</td>
										<td class="text-white" class="text-center"><a href="?page=backend&menu=manageuser&id='.$user_w['id'].'">'.$user_w['username'].'</a></td>
										<td class="text-white" class="text-center">'.$user_w['ip'].'</td>
										<td class="text-white" class="text-center">'.$user_w['points'].'/'.$user_w['rp'].'/'.$user_w['eve'].'</td>
										<td class="text-white" class="text-center">'.$user_w['status'].'</td>
									</tr>
								';
							}
						}
