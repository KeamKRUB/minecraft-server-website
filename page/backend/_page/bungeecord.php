			...
			<?php
				...
				if($query_list_bungeecord->num_rows != 0)
				{
					while($list_bungeecord = $query_list_bungeecord->fetch_assoc())
					{
						...
						echo '
							<tr>
								<td class="text-left text-light">'.$list_bungeecord['id'].'</td>
								<td class="text-center text-light">'.$list_bungeecord['name_server'].'</td>
								<td class="text-center text-light"><a href="?page=backend&menu=bungeecord&id='.$list_bungeecord['id'].'" class="btn btn-primary">แก้ไข #'.$list_bungeecord['id'].'</a></td>
							</tr>
						';
					}
				}
