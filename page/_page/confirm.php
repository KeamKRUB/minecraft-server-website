								...
								?>
									<div class="col-md-12 mb-3">
										<?php
											...
											{
												$msg = 'TruePoint คุณไม่เพียงพอ!';
												$alert = 'error';
												$msg_alert = 'เกิดข้อผิดพลาด!';
											}
											elseif($player['rp'] < $buy['pricerp'])
											{
												$msg = 'RealPoint คุณไม่เพียงพอ!';
												$alert = 'error';
												$msg_alert = 'เกิดข้อผิดพลาด!';
											}
											elseif($player['eve'] < $buy['priceeve'])
											{
												$msg = 'EventPoint คุณไม่เพียงพอ ';
												$alert = 'error';
												$msg_alert = 'เกิดข้อผิดพลาด!';
											}
											else
											{
												...
												if($query_rcon_server->num_rows > 0)
												{
													...
													$sql_rem_rp = 'UPDATE authme SET rp = rp-"'.$buy['pricerp'].'" WHERE id = "'.$_SESSION['uid'].'"';
													$query_rem_rp = $connect->query($sql_rem_rp);
													$sql_rem_eve = 'UPDATE authme SET eve = eve-"'.$buy['priceeve'].'" WHERE id = "'.$_SESSION['uid'].'"';
													$query_rem_eve = $connect->query($sql_rem_eve);
														...
														if($query_rem_points)
														{
                                                            ...
                                                            $rcon->sendCommand("m ".$_SESSION['username']." คุณได้ทำการสั่งซื้อ ".$buy['name']."");
